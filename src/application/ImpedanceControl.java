...
/**
 * Description: This program implemented impedance control with the smartServo interface
...
public class ImpedanceControl extends RoboticsAPIApplication {
    ...
    int _milliSleepToEmulateComputationalEffort = 10;
	...
	public void run(){
    	...
    	//=============================================================================================//
		moveToInitialPosition(); //moving to the initial position
		...
		// Set up the SmartServo interface 
		...
		double UL = 3, LL = -3; //dead zone for fx and fy
		...
		for (double i = 0; i < _numRuns; ++i) {
			...
			//ForceY = 0; //Rotation only
			//TorqueZ = 0; //Translation only
			
			// Integrating differential equations to get reference position
			startTimeStamp = System.currentTimeMillis();
			dt = (double) ((startTimeStamp - endTimeStamp) / 1000.0); //time period in second
	 ...
	 public void moveToInitialPosition()
	    {
	        lbr.move(
	                ptp(Math.toRadians(0), Math.PI / 180 * 0., 0., -Math.PI / 180 * 80., 0.,
	                        Math.PI / 180 * 100, 0.).setJointVelocityRel(0.2));
	        /*
