...
/**
 * Description: This program arms to create different compliance behavior
 * corresponding to the estimation of the operator's intention(rotation vs. translation)
 * 1. Running the robot with compliant reaction by using smartServor interface
 * 2. Check if the Estimation == rotation: the robot will be easy to rotate
 * 	  		if the Estimation == translation: the robot will be easy to translate
 *    Switching function is based on Force Magnitude
 *  
 *  Author: Vinh Quang Nguyen (vinhnguyen.ac@gmail.com)
 *  Version: July 04, 2016
 */

public class RotationVSTranslation_ForceMag extends RoboticsAPIApplication {
    ...
    int _milliSleepToEmulateComputationalEffort = RotationVSTranslation_2HMMs._milliSleepToEmulateComputationalEffort;
    ...
    // Initialize the KUKA controller
	...
	public void run(){
		...
		for (double i = 0; i < _numRuns; ++i) {
			...
			// Switching mode base on the results of intention recognition in background task SwitchingbasedForceMag
			if (SwitchingbasedForceMag.prediction_SF == 0) { // translation
				TorqueZ = 0;
			}

			if (SwitchingbasedForceMag.prediction_SF == 1) { // rotation
				ForceY = 0;
			}
			...
			// Set new destination for the end-effector 
	...
	public static void main(String[] args) {
		RotationVSTranslation_ForceMag app = new RotationVSTranslation_ForceMag();
		app.runApplication();
	}
}
